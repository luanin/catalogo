...
void main() {
	...
	if((height - gl_FragCoord.y) + scrollY < height * 1.){
		if(p.y < wavePos(1., p, numWaves)) color = vec3(86./255., 32./255., 11./255.);
		else if(p.y < wavePos(2., p, numWaves)) color = vec3(123./255., 44./255., 19./255.);
		else if(p.y < wavePos(3., p, numWaves)) color = vec3(177./255., 68./255., 36./255.);
		else if(p.y < wavePos(4., p, numWaves)) color = vec3(211./255., 87./255., 33./255.);
		else if(p.y < wavePos(5., p, numWaves)) color = vec3(241./255., 126./255., 35./255.);
		else if(p.y > wavePos(5., p, numWaves)) color = vec3(251./255., 161./255., 28./255.);

		if(p.y < wavePos(4., p, numWaves)){
			if(
				distance(squareP, truePScale / 2.) < .2
				&& distance(squareP, (truePScale +  vec2(truePScale.x * .1, truePScale.y * .1)) / 2.) > .2
